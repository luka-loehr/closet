...
const VARIANT_LABEL: Record<string, string> = { white: "White", dark: "Black" };
...
let addOwn = false;
const MODE_HINT = { own: "Goes into your closet as two studio views; use it to complete fits.", tryon: "Generates you wearing it in white and black." };
...
async function startAdd(src: { file?: File; url?: string; dataUrl?: string }) {
  ...
  form.innerHTML = `
    ...
    <div class="row" style="margin-top:22px"><button class="btn" id="submit">${own ? "Add to closet" : "Generate looks"}</button><button class="btn ghost" id="cancel" type="button">Discard</button><span class="muted" style="font-size:12px">${own ? "Two studio views on white are generated from your photo in the background (~40 s): the piece and a detail shot, or for shoes the side and three-quarter view. Your photo itself is never shown." : "Both looks come from one generation (~40 s). You can leave right away."}</span></div>`;
