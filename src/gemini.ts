...
- category: one of ${CATEGORIES.join(", ")}. A matching top and bottom is "set".
- colors: 1 to 3 main colours as short lowercase colour words, most dominant first (e.g. "navy", "off-white", "light grey"). One word for a single-colour piece.
- description: one sentence about cut, fabric and notable details. No marketing language.
...
export async function analyzeGarment(env: Env, image: ImageInput): Promise<Analysis> {
  ...
  try {
    ...
    return {
      ...
      colors: Array.isArray(p.colors) ? p.colors.slice(0, 3).map((c: unknown) => String(c).toLowerCase().trim().slice(0, 30)).filter(Boolean) : [],
